...
class InsertBrand implements NotifyHandlerInterface
{
    ...
    public function execute(array $registers): void
    {
        if (!empty($registers)) {
            ...
            foreach ($registers as $data) {
                if ($data['event'] == self::INSERT && $data['resource_type'] == self::RESOURCE_BRAND) {
                    ...
                    $this->productService->addOptionToAttribute($optionLabel, self::ATTRIBUTE_CODE_BRAND);
                    $this->notifyOmnikDataInterface->changeStatusNotify((int) $data['entity_id']);
