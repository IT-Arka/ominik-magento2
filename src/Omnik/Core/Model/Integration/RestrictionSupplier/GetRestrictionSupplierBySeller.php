...
namespace Omnik\Core\Model\Integration\RestrictionSupplier;

use Omnik\Core\Api\ConfigInterface;
use Omnik\Core\Model\AbstractIntegration;

class GetRestrictionSupplierBySeller extends AbstractIntegration
{
    /**
     * @param string $seller
     ...
     * @throws \Exception
     */
    public function execute(string $seller, string $supplier, int $storeId)
    {
        $client = $this->getClient($storeId);
        $this->config->set(ConfigInterface::PARAM_SELLER, $seller);
        $client->setConfig($this->config);

        return $client->getRequest(self::PATH_GET_RESTRICTION_SUPPLIER . $supplier);
    }
}
